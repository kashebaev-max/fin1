...
export default function AuthForm({ mode, blocked, confirmationRequired }: { mode?: string; blocked?: boolean; confirmationRequired?: boolean }) {
  ...
  }, [blocked, confirmationRequired, supabase]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        void trackEvent("login_success", "activation");
        ...
        void trackEvent("signup_submit", "activation");
        ...
        void trackEvent("signup_response", "activation", { has_session: Boolean(data.session) });
    ...
    <div className="w-full max-w-md">
      ...
      <div className="rounded-2xl p-8" style={{ background: "var(--card)", border: "1px solid var(--brd)" }}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {error && (
            <div role="alert" className="text-sm p-3 rounded-lg" style={{ background: "#EF444420", color: "#EF4444" }}>
              {error}
            </div>
          )}
          {success && (
          ...
          {(pendingConfirm || (REQUIRE_EMAIL_CONFIRMATION && isLogin)) && (
            <button
              type="button"
