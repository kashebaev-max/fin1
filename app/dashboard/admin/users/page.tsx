  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== "all") {
      result = result.filter(u => {
        ...
        if (filter === "admin") return u.is_platform_admin === true;
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[12px]" style={{ color: "var(--t3)" }}>
            Клиенты Finstat.kz — отдельные организации, не сотрудники вашей компании
          </p>
        </div>
        ...
        <table className="w-full text-[11px]" style={{ borderCollapse: "collapse" }}>
          <thead style={{ background: "var(--bg)", position: "sticky", top: 0 }}>
            <tr style={{ borderBottom: "1px solid var(--brd)" }}>
              ...
              <th className="text-left p-3" style={{ color: "var(--t3)" }}>Аккаунт</th>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={u.id} className="cursor-pointer hover:opacity-80"
                  ...
                  <td className="p-3">
                    {u.is_blocked ? (
                      <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: "#EF444415", color: "#EF4444" }}>ЗАБЛОК.</span>
                    ) : u.is_platform_admin ? (
                      <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: "#F59E0B20", color: "#F59E0B" }}>АДМИН</span>
                    ) : (
                      <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: "#6B728015", color: "#6B7280" }}>КЛИЕНТ</span>
                    )}
                  </td>
                  <td className="p-3">
                    {!sub ? (
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div className="rounded-lg p-3 mb-3" style={{ background: "var(--bg)" }}>
              <div className="grid grid-cols-2 gap-3 text-[12px]">
                ...
                <div>
                  <div className="text-[10px]" style={{ color: "var(--t3)" }}>Аккаунт</div>
                  <div className="font-semibold">
                    {selectedUser.user.is_blocked ? "Заблокирован" : selectedUser.user.is_platform_admin ? "Админ платформы" : "Клиент"}
                  </div>
                </div>
                <div>
            ...
            {actionMsg && (
              <div className="text-[12px] font-semibold mb-2 p-2 rounded" style={{ background: "var(--bg)" }}>{actionMsg}</div>
            )}
            ...
            {!selectedUser.user.is_platform_admin && (
              <div className="grid grid-cols-2 gap-2 mt-3">
                {selectedUser.user.is_blocked ? (
                  ...
                  <button disabled={actionLoading} onClick={() => runAdminAction("block", selectedUser.user.id)}
                    className="py-2 rounded-lg text-[11px] font-bold" style={{ background: "#EF444420", color: "#EF4444", border: "none" }}>
                    Заблокировать
                  </button>
                )}
                <button disabled={actionLoading} onClick={() => runAdminAction("extend_trial", selectedUser.user.id, 30)}
                ...
                <button disabled={actionLoading} onClick={() => runAdminAction("delete", selectedUser.user.id)}
                  className="col-span-2 py-2 rounded-lg text-[11px] font-bold" style={{ background: "#EF444430", color: "#991B1B", border: "none" }}>
